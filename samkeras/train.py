# sam2_tfkeras/main.py

import tensorflow as tf
from sam2_tfkeras.modeling.sam2_base import SAM2Base
...
from sam2_tfkeras.modeling.memory_attention import MemoryAttention, MemoryAttentionLayer
from sam2_tfkeras.modeling.memory_encoder import MemoryEncoder, MaskDownSampler, CXBlock, Fuser
from sam2_tfkeras.modeling.position_encoding import PositionEmbeddingSine
from sam2_tfkeras.modeling.sam.transformer import RoPEAttention, Attention, TwoWayTransformer
...
from sam2_tfkeras.modeling.sam2_utils import LayerNorm2d, DropPath, MLP, get_1d_sine_pe, select_closest_cond_frames
import os
import cv2
import tarfile
import numpy as np
...
def train_step(images, masks, prompts):
    ...
    optimizer.apply_gradients(zip( fgradients, sam_model.trainable_variables))
...
for epoch in range(epochs):
    for batch, (images, masks, prompts) in enumerate(train_dataset):
        ...
        print(f"Epoch: {epoch+1}, Batch: {batch+1}, Loss: {loss.numpy():.4f}, Recall@50: {recall_50.numpy():.4f}, Recall@75: {recall_75.numpy():.4f}, Recall@90: {recall_90.numpy():.4f}")
