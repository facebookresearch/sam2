...
class MemoryAttentionLayer(layers.Layer):
    ...
    def __init__(
        ...
        cross_attention: layers.Layer,  # TensorFlow attention layer for cross-attention
        ...
        self_attention: layers.Layer,  # TensorFlow attention layer for self-attention
    ):
...
class MemoryAttention(layers.Layer):
    ...
    def __init__(
        ...
        num_layers: int,  # Number of MemoryAttentionLayers
        batch_first: bool = True,  # Whether the batch dimension is first 
    ):
    ...
    def call(
        self,
        curr: tf.Tensor,   # Current frame features
        memory: tf.Tensor,  # Memory features
        curr_pos: Optional[tf.Tensor] = None,  # Positional encodings for current features
        memory_pos: Optional[tf.Tensor] = None,  # Positional encodings for memory 
        ...
        Args:
            curr (tf.Tensor): Current frame features.
            memory (tf.Tensor): Memory features.
            curr_pos (tf.Tensor, optional): Positional encodings for current features. Defaults to None.
            memory_pos (tf.Tensor, optional): Positional encodings for memory features. Defaults to None.
            num_obj_ptr_tokens (int, optional): Number of object pointer tokens. Defaults to 0.
            training (bool, optional): Whether in training mode. Defaults to False.

        Returns:
            tf.Tensor: Memory-conditioned features
        """
        ...
        # Add positional encoding to the input if enabled
        if self.pos_enc_at_input and curr_pos is not None:
            output = output + 0.1 * curr_pos

        if self.batch_first:  
            # Convert to batch-first format (B, T, C)
        ...
        # Apply each MemoryAttentionLayer 
        ...
        # Convert back to sequence-first format (T, B, C)
        if self.batch_first:  
